...
ダッシュボードと一覧を、LINEなどへ画像・文章で送れる。
画像は Canvas に直接描いている（外部ライブラリを入れない方針のため）。
**地図は共有できない**（タイルが別ドメインのため canvas が汚染される）。

**既定では氏名・連絡先・掲示住所を含めない。** 除外リストで消すのではなく、
そもそも組み立てない。含めるかはその都度選ぶ（ダッシュボードはスイッチ、
一覧は送る直前の確認）。既定を開いたままにすると、入れたことを忘れて送る事故が起きる。

送ったものは相手の端末とトーク履歴に残り、転送もでき、取り消せない。

- `share.js` … 何を送るかの組み立て（純粋関数・テスト対象）
- `share-image.js` … Canvas への描画（Node に canvas が無く検査できないため薄く保つ）
